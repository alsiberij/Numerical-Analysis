...
void NewtonSolver::solve() {
    double **JF = new double*[dim];                 //Матрица JF - матрица Якоби
    for (int i = 0; i < dim; ++i) {
        JF[i] = new double[dim + 1];
    }

    double sig1;    //Критерии завершения вычислений
    double sig2;
    ...
    while (true) {
        ...
        for (int i = 0; i < dim; ++i) {             //Заполнения матрицы Якоби
        ...
        GaussianSolver solver(JF, dim);   //Решение системы линейных уравнений с матрицей Якоби
        solver.solve();
        ...
        sig1 = functions[0]->F(solutions);               //Первый критерий завершения
        for (int i = 0; i < dim; ++i) {
